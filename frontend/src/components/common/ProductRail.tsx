  ...
  return (
    ...
    >
      <div className={isSpotlight ? 'mb-3 flex items-center justify-between px-2 pt-1' : 'flex items-center justify-between'}>
        <h2 className={isSpotlight ? 'text-2xl font-extrabold text-white md:text-3xl' : 'text-2xl font-extrabold text-slate-950'}>{title}</h2>
        <Link
          href="/products"
          aria-label={`View all ${title}`}
          className={
            isSpotlight
              ? 'grid h-9 w-12 place-items-center rounded-full bg-white text-slate-950 shadow-sm transition hover:scale-105'
              : 'grid h-12 w-12 place-items-center rounded-full bg-white text-slate-950 shadow-sm ring-1 ring-slate-200 transition hover:-translate-y-0.5 hover:shadow-md'
          }
        >
      ...
      <div className={isSpotlight ? 'scrollbar-none flex gap-2 overflow-x-auto rounded-2xl bg-white p-2 md:gap-3' : 'scrollbar-none flex gap-4 overflow-x-auto'}>
...
function ProductTile({ product, spotlight }: { product: Product; spotlight: boolean }) {
  const image = product.images?.[0] || 'https://images.unsplash.com/photo-1523275335684-37898b6baf30?auto=format&fit=crop&w=900&q=80';
  ...
  if (spotlight) {
    return (
      <Link href={`/products/${product.id}`} className="group min-w-[245px] flex-1 md:min-w-[260px]">
        <article className="overflow-hidden rounded-lg bg-white transition group-hover:-translate-y-0.5">
          <div className="relative aspect-[1.48] overflow-hidden rounded-lg bg-slate-50">
          ...
          <div className="pt-2">
            <h3 className="line-clamp-1 text-base font-medium text-slate-950 md:text-lg">{product.category}</h3>
            <p className="line-clamp-1 text-base font-extrabold text-slate-950 md:text-lg">
